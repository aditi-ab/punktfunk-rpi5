...
- **HDR (10-bit, BT.2020 PQ).** With HDR on, the session carries the 10-bit BT.2020 PQ signal in
  16-bit planes and adds ~15 % to the bitrate. Available on Windows and Linux hosts — on Linux it
  needs the same HDR source as the other codecs (a `punktfunk-gamescope` virtual output; see
  [the HDR chain](/docs/hdr#per-host)). It combines with 4:4:4 on both.
- **10-bit SDR.** A client that asks for 10-bit but not HDR gets the same 16-bit plane depth with
  ordinary BT.709 colour — on Linux too, since the colour conversion is PyroWave's own.
...
5.3 Gbps, which is more than a 5GbE link carries. An Automatic session catches this itself:
the bring-up ramp measures the link before the first frame and drops the pin to fit — a 2.5GbE
path settles near 1.7 Gbps. A client bitrate does not lower it: PyroWave ignores it. When the
measurement can't help — an older host, or a ceiling you want tighter than the link's wall —
cap the host's pin with [`PUNKTFUNK_PYROWAVE_MAX_MBPS`](/docs/configuration).
...
3. Leave the bitrate on Automatic: a PyroWave session pins itself to the ~1.6 bpp rate for
   your mode (≈200 Mbps at 1080p60; ~2.6 bpp for 4:4:4, +15 % for HDR). Before the first
   frame the client runs the same bring-up ramp the adaptive bitrate uses and fits the pin to
   it: only a measured link wall moves it, down to ~0.7× what the ramp proved — it never
   rises, and a ramp that measured nothing (or an older host that never ran one) leaves the
   pin exactly where it resolved. A client bitrate is ignored, and the adaptive-bitrate
   controller stays off — this codec has no useful low-rate regime, so under sustained loss
   the right move is switching back to HEVC, not degrading. The pin follows the resolution:
   a mid-stream resize (e.g. Match window) re-pins the rate for the new mode, so resizing a
   window down also cuts the bandwidth.
...
- PyroWave HDR on **Linux** needs the same HDR source as the other codecs — a `punktfunk-gamescope`
  virtual output ([the HDR chain](/docs/hdr#per-host)). Without it the session is SDR, as it is for
  HEVC and AV1.
