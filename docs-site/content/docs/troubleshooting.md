...
The Windows host is two separate things: the **`PunktfunkHost` service** (streaming) and the
**`PunktfunkWeb` scheduled task** (the console). One can be down while the other is fine, so check
