...
install, or move to a row and edit it. Without one — piped, in CI, or with `--yes` — it prints the
same choices as plain lines and takes the defaults.

The defaults follow the box. Bazzite and Nobara linger; a seatless SSH session lingers; an active
Sunshine/Apollo host opts into Moonlight compat; the clipboard stays off. The full controller
...
`PUNKTFUNK_INSTALL_OMARCHY_SETUP`, `PUNKTFUNK_INSTALL_MGMT_PORT`, `PUNKTFUNK_INSTALL_DRY_RUN`).
...
It covers Ubuntu/Debian, Fedora, Arch-family and Bazzite/Fedora Atomic; NixOS, SteamOS and Windows
it points at their pages. **Preview:** the per-system pages above remain the documented path, and
