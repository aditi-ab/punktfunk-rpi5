...
| **Bar widget** *(optional)* | the Punktfunk Quickshell widget in the bar — live status, pairing approvals and devices without opening the console. |
...
| **Client menu** *(optional)* | when `punktfunk-client` is installed: your saved hosts as connect and wake rows in the same Punktfunk submenu. |

Every *(optional)* step defaults to **yes** — Enter accepts, `n` declines, and a run without a
terminal takes the defaults, so the guided installer delivers the full integration on its own.
...
webapp, the bar widget, the hooks, the theme template — and restores the screen-share picker. Your config and pairings survive both; see
