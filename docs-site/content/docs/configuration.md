...
| `PUNKTFUNK_OSD_SCALE` | multiplier, e.g. `1.5` *(default: the **Overlay size** setting)* | Overrides [Overlay size](/docs/client-settings#overlay) for one launch. Size of the in-stream overlay — the stats OSD, the capture hint and the start banner. They already follow your display's scaling setting (200 % display → twice the pixels), so set this only to nudge that: bigger for a TV across the room, smaller if your compositor reports an aggressive scale. Clamped to 0.5×–4×, and a line that would run off the screen is shrunk to fit. |
