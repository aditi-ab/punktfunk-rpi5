...
export const ActionInfo = Schema.Struct({ "available": Schema.Boolean.annotate({ "description": "Whether this host can run it right now (platform probe — a VM that can't S3 lists\nsleep as unavailable rather than offering a dead switch)." }), "danger": Schema.Boolean.annotate({ "description": "Whether a client UI should double-confirm (the action loses state — reboot/shutdown)." }), "group": Schema.String.annotate({ "description": "Action group (`power` for the built-ins)." }), "id": Schema.String.annotate({ "description": "Stable action id (`power.sleep`, …) — the invoke path parameter." }), "permitted": Schema.Boolean.annotate({ "description": "Whether THIS caller may invoke it (admin lane: always; cert lane: the `GRANT_POWER`\nbit of the device's live access mask)." }), "title": Schema.String.annotate({ "description": "Display title. Clients localize known ids and fall back to this for unknown ones." }), "unavailable_reason": Schema.optionalKey(Schema.Union([Schema.String, Schema.Null]).annotate({ "description": "Why it is unavailable, when it is." })) }).annotate({ "description": "One action as the caller sees it (`GET /actions`)." })
...
export const ArmNativePairing = Schema.Struct({ "expires_in_secs": Schema.optionalKey(Schema.Never), "fingerprint": Schema.optionalKey(Schema.Union([Schema.String, Schema.Null]).annotate({ "description": "Optional: bind the window to ONE device fingerprint (hex SHA-256, e.g. from a pending knock).\nWhen set, only a pairing attempt from that fingerprint consumes the window — so an unpaired\nLAN peer can neither pair nor burn a window armed for a specific device (security-review #9).\nOmit for an unbound window (any device may use the PIN — trusted-LAN only)." })), "grants": Schema.optionalKey(Schema.Never), "ttl_secs": Schema.optionalKey(Schema.Never) }).annotate({ "description": "Arm-native-pairing request body." })
...
export const ClientLogMeta = Schema.Struct({ "device_name": Schema.String.annotate({ "description": "The paired device's name at upload time (sanitized for the filesystem)." }), "fingerprint_prefix": Schema.String.annotate({ "description": "First 16 hex chars of the device's pairing fingerprint — enough to correlate with the\npaired-devices roster without repeating the full identity in every filename." }), "id": Schema.String.annotate({ "description": "The bundle id (its filename stem) — pass to the fetch/delete endpoints." }), "received_ms": Schema.Number.annotate({ "description": "Upload time (unix ms, from the file's mtime).", "format": "int64" }).check(Schema.isInt()).check(Schema.isGreaterThanOrEqualTo(0)), "size_bytes": Schema.Number.annotate({ "description": "Bundle size in bytes.", "format": "int64" }).check(Schema.isInt()).check(Schema.isGreaterThanOrEqualTo(0)) }).annotate({ "description": "One stored bundle, as the console lists it." })
export type ClientLogUploaded = { readonly "id": string }
export const ClientLogUploaded = Schema.Struct({ "id": Schema.String.annotate({ "description": "The stored bundle's id." }) }).annotate({ "description": "Response to a successful upload." })
...
export const Health = Schema.Struct({ "abi_version": Schema.Number.annotate({ "description": "`punktfunk-core` C ABI version.", "format": "int32" }).check(Schema.isInt()).check(Schema.isGreaterThanOrEqualTo(0)), "status": Schema.String.annotate({ "description": "Always `\"ok\"` when the host responds." }), "version": Schema.String.annotate({ "description": "`punktfunk-host` crate version." }) }).annotate({ "description": "Liveness + version probe." })
export type HiddenState = { readonly "hidden": boolean, readonly "id": string }
export const HiddenState = Schema.Struct({ "hidden": Schema.Boolean.annotate({ "description": "Its visibility after the call." }), "id": Schema.String.annotate({ "description": "The entry id the call addressed." }) }).annotate({ "description": "What `setLibraryEntryHidden` echoes back." })
export type HiddenToggle = { readonly "hidden": boolean }
export const HiddenToggle = Schema.Struct({ "hidden": Schema.Boolean.annotate({ "description": "Whether this title should be hidden from every play surface." }) }).annotate({ "description": "Request body for `setLibraryEntryHidden`." })
...
export const Identity = Schema.Literals(["shared", "per-client", "per-client-mode"]).annotate({ "description": "Stable display identity, so desktop environments persist per-display config (KDE scaling). The\nslot this resolves to is carried per backend: the Windows EDID serial + IddCx connector index,\nKWin's per-slot output name, and the host-persisted Mutter scale map." })
...
export const LayoutMode = Schema.Literals(["auto-row", "manual"]).annotate({ "description": "How group members are arranged in the desktop coordinate space, resolved by `layout::arrange` —\nwhich both the `/display/state` readout and (on Linux, KWin only) the per-backend position apply\nconsume, so the answer is computed in exactly one place." })
...
export const NativePairStatus = Schema.Struct({ "armed": Schema.Boolean.annotate({ "description": "True while a pairing window is open." }), "enabled": Schema.Boolean.annotate({ "description": "Whether the native host is running (the unified host started with `--native`)." }), "expires_in_secs": Schema.optionalKey(Schema.Never), "paired_clients": Schema.Number.annotate({ "description": "Number of paired native clients.", "format": "int32" }).check(Schema.isInt()).check(Schema.isGreaterThanOrEqualTo(0)), "pin": Schema.optionalKey(Schema.Union([Schema.String, Schema.Null]).annotate({ "description": "The PIN to display while armed (null when disarmed)." })) }).annotate({ "description": "Native (punktfunk/1) pairing status. Unlike GameStream, the **host** mints the PIN (the SPAKE2\nceremony needs it client-side first), so the console **displays** `pin` for the user to enter on\ntheir device — armed on demand for a short window." })
...
export const PluginLogLine = Schema.Struct({ "level": Schema.String.annotate({ "description": "`ERROR` | `WARN` | `INFO` | `DEBUG` | `TRACE`. Anything else is coerced to `INFO`." }), "msg": Schema.String, "source": Schema.String.annotate({ "description": "Which unit emitted it — a plugin's `definePlugin` name, a package name, or `runner`.\nSurfaced in the console's target column as `plugin:<source>`." }), "ts_ms": Schema.Number.annotate({ "description": "When the line was produced, unix milliseconds. Kept verbatim — see\n[`crate::log_capture::LogRing::push_remote`].", "format": "int64" }).check(Schema.isInt()).check(Schema.isGreaterThanOrEqualTo(0)) }).annotate({ "description": "One log line produced by the runner or a plugin inside it (`POST /plugins/logs`)." })
...
export const ProviderRunningAccepted = Schema.Struct({ "matched": Schema.Number.annotate({ "description": "How many reported titles matched an entry this provider currently publishes." }).check(Schema.isInt()).check(Schema.isGreaterThanOrEqualTo(0)), "ttl_s": Schema.Number.annotate({ "description": "Seconds this report stays authoritative without being restated — re-report inside it while\nanything is running.", "format": "int64" }).check(Schema.isInt()).check(Schema.isGreaterThanOrEqualTo(0)), "unknown": Schema.Number.annotate({ "description": "How many were ignored because no such entry exists (a report that raced a reconcile)." }).check(Schema.isInt()).check(Schema.isGreaterThanOrEqualTo(0)) }).annotate({ "description": "The result of a liveness report." })
...
export const RenameClient = Schema.Struct({ "label": Schema.optionalKey(Schema.Union([Schema.String, Schema.Null]).annotate({ "description": "The name to show for this device. `null` (or an empty/whitespace-only string) clears it and\nthe device goes back to being listed by fingerprint alone.\n\nScrubbed before storage by the same sanitizer the native plane runs on device names:\ncontrol characters and Unicode bidi overrides are stripped (they could make one paired\ndevice impersonate another in this very list), whitespace collapsed, and the result capped\nat 64 characters." })) }).annotate({ "description": "Body of `PATCH /clients/{fingerprint}` — the device's display name." })
export type RunningTitle = { readonly "external_id": string, readonly "pid"?: never }
export const RunningTitle = Schema.Struct({ "external_id": Schema.String.annotate({ "description": "The provider's own stable id for the title — the same key its reconcile payload uses." }), "pid": Schema.optionalKey(Schema.Never) }).annotate({ "description": "One running title in a provider's liveness report." })
...
export const ScannerToggle = Schema.Struct({ "enabled": Schema.Boolean.annotate({ "description": "Whether this source should contribute titles on this host." }) }).annotate({ "description": "Request body for `setLibraryScanner`." })
...
export type SessionSettings = { readonly "disconnect_grace_seconds"?: number, readonly "game_on_new_launch"?: "keep" | "end", readonly "game_on_session_end"?: "keep" | "on_quit" | "always", readonly "session_on_game_exit"?: boolean, readonly "version"?: number }
...
export type SessionSettingsState = { readonly "configured": boolean, readonly "enforced": ReadonlyArray<string>, readonly "settings": { readonly "disconnect_grace_seconds"?: number, readonly "game_on_new_launch"?: "keep" | "end", readonly "game_on_session_end"?: "keep" | "on_quit" | "always", readonly "session_on_game_exit"?: boolean, readonly "version"?: number } }
...
export const PluginLogBatch = Schema.Struct({ "entries": Schema.Array(PluginLogLine) }).annotate({ "description": "A batch of runner log lines." })
...
export const PluginSummary = Schema.Struct({ "category": Schema.optionalKey(Schema.Union([Schema.String, Schema.Null]).annotate({ "description": "The plugin's kind — see [`PluginRegistration::category`]." })), "id": Schema.String, "title": Schema.String, "ui": Schema.optionalKey(Schema.Union([Schema.Null, PluginUiPublic], { mode: "oneOf" })), "version": Schema.optionalKey(Schema.Union([Schema.String, Schema.Null])) }).annotate({ "description": "One entry in `GET /plugins`. **Never carries the secret** — the browser learns a plugin exists\nand has a UI, nothing that lets it reach the plugin directly (it goes through the console proxy)." })
...
export const DiagnosticsReport = Schema.Struct({ "checks": Schema.Array(HostCheck).annotate({ "description": "Every registered check, worst-first. Includes `ok` and `inapplicable` rows — the console\ndecides what to hide, because \"what's working\" is the reassurance the dashboard omits." }), "ran_at_unix": Schema.Number.annotate({ "description": "When the probes last ran (unix seconds).", "format": "int64" }).check(Schema.isInt()).check(Schema.isGreaterThanOrEqualTo(0)) }).annotate({ "description": "The `GET /diagnostics` body." })
