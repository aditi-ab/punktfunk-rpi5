...
public enum VirtualPad {
    ...
    static let narrow: Float = 720
}

private func disc(_ label: String, _ glyph: String, _ bit: UInt32, _ cx: Float, _ cy: Float, _ r: Float) -> PadControl {
    PadControl(label: label, rect: PadRect(cx - r, cy - r, 2 * r, 2 * r),
...
public func padControls(layout: String, w: Float, h: Float) -> [PadControl] {
    ...
    var out: [PadControl] = []
    if sticks {
        out.append(disc("Left bumper", "LB", GamepadWire.leftShoulder, P.margin + P.bumperRadius, P.margin + P.bumperRadius, P.bumperRadius))
        out.append(PadControl(label: "Left trigger",
                              rect: PadRect(P.margin, P.margin + 2 * P.bumperRadius + 8, P.triggerW, P.triggerH),
                              kind: .trigger(axis: GamepadWire.axisLT)))
        out.append(disc("Right bumper", "RB", GamepadWire.rightShoulder, w - P.margin - P.bumperRadius, P.margin + P.bumperRadius, P.bumperRadius))
        out.append(PadControl(label: "Right trigger",
                              rect: PadRect(w - P.margin - P.triggerW, P.margin + 2 * P.bumperRadius + 8, P.triggerW, P.triggerH),
                              kind: .trigger(axis: GamepadWire.axisRT)))
        out.append(PadControl(label: "Left stick",
    ...
    if face {
        ...
        out.append(PadControl(label: "D-pad", rect: dpad, kind: .dpad))
        let c = P.faceHit / 2
        let gap: Float = 40
    ...
    if sticks {
        ...
        out.append(PadControl(label: "Right stick", rect: right,
    ...
    out.append(disc("Select", "⧉", GamepadWire.back, w / 2 - 64, midY, P.smallRadius))
    out.append(disc("Guide", "◎", GamepadWire.guide, w / 2, midY, P.smallRadius))
    out.append(disc("Start", "☰", GamepadWire.start, w / 2 + 64, midY, P.smallRadius))
    return out
}
