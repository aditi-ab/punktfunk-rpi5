...
// The menu-event vocabulary + synthesizer and the pad descriptors live in the portable
// `menu_nav` module (the Skia console drives them from raw pad samples on Android too);
// re-exported here so every desktop `gamepad::MenuEvent` spelling stays valid.
